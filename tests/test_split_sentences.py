...
def test_decimal_not_split_into_separate_chunks(tokenizer: SentencePieceTokenizer):
...
def test_multiple_decimals_preserved(tokenizer: SentencePieceTokenizer):
...
def test_money_not_split_into_separate_chunks(tokenizer: SentencePieceTokenizer):
...
def test_decimals_are_not_split_on_period(tokenizer: SentencePieceTokenizer):
    ...
    On this branch decimals are rewritten to spoken form by ``normalize_text``
    before tokenisation, so the chunk carries ``98 point 6`` rather than ``98.6``.
    """
    text = (
...
def test_multiple_decimals_in_one_sentence(tokenizer: SentencePieceTokenizer):
    ...
    assert len(chunks) == 1
    assert "3 point 14" in chunks[0]
    assert "2 point 718" in chunks[0]
...
def test_decimal_followed_by_sentence_boundary(tokenizer: SentencePieceTokenizer):
    ...
    assert "98 point 6" in rejoined
    assert "3 point 14" in rejoined
    assert "2 point 718" in rejoined
